...
public class ServerTextTranslateApplication {
	...
	public static void main(String[] args) throws IOException
	{
		...
		TextForTranslated setupText = new TextForTranslated();
		setupText.createData();
		
		// create server socket object
		ServerSocket serverSocket = null;
		TextTranslator textTranslate = new TextTranslator();
		...
		try
		{
			...
			while(true)
			{
				...
				{
					translated = textTranslate.translateToBM(text);
				}
				
				// if target language is Arabic
				if(language.equals(arb))
				{
					translated = textTranslate.translateToArb(text);
				}
				
				// if target language is Korean
				if(language.equals(krn))
				{
					translated = textTranslate.translateToKrn(text);
				}
